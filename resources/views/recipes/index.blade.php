        ...
        <div class="bg-white bg-opacity-80 backdrop-blur-md p-6 rounded-xl shadow-xl">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white/90 backdrop-blur-md shadow-xl rounded-2xl overflow-hidden transform hover:-translate-y-1 hover:shadow-2xl transition duration-300 border border-gray-200">
                    <img src="{{ asset('storage/' . $recipe->image) }}" class="w-full h-60 object-cover" alt="{{ $recipe->title }}">
                    
                    <div class="p-5">
                        <h5 class="text-2xl font-bold text-gray-800">{{ $recipe->title }}</h5>
                        <p class="text-gray-600 mt-2 text-sm">{{ \Str::limit($recipe->description, 100) }}</p>
                        
                        <a href="{{ route('recipes.show', $recipe) }}" class="mt-4 inline-block bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium py-2 px-5 rounded-lg hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 shadow-md">
                            View Recipe
                        </a>
                    </div>
                </div>
